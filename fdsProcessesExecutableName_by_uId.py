...
def fdsProcessesExecutableName_by_uId():
  ...
  assert hProcessesSnapshot != INVALID_HANDLE_VALUE, \
      fsGetErrorMessage("CreateToolhelp32Snapshot(0x%08X, 0)", TH32CS_SNAPPROCESS);
  ...
  assert HRESULT_FROM_WIN32(uProcess32_WError) == ERROR_NO_MORE_FILES, \
      fsGetErrorMessage("Process32%sW(0x%08X, ...)" % \
      (bFirstProcess and "First" or "Next", hProcessesSnapshot.value,), uProcess32_WError);
  assert KERNEL32.CloseHandle(hProcessesSnapshot), \
      fsGetErrorMessage("CloseHandle(0x%08X)" % (hProcessesSnapshot.value,));
